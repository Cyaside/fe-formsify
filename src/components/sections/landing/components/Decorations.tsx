...
function SectionWrapper({ className, style, children }: { className?: string; style?: any; children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (section) setMountedInSection(true);
    else {
      // If not used inside a section, don't render and warn for developer.
      // eslint-disable-next-line no-console
      console.warn("Decoration mounted outside a <section>. Decorations are intended to be used per-section.");
...
export function DecoLeft({ className, style }: { className?: string; style?: any } = {}) {
...
export function DecoRight({ className, style }: { className?: string; style?: any } = {}) {
